...
const SingleProject = ({ project }) => {
  const { name, description, tags, code, demo, image, features, role } = project;

  // Check if image is an external URL (starts with http or https)
  const isExternalImage = image && image.src && (image.src.startsWith('http://') || image.src.startsWith('https://'));
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        ...
        <div className="p-6 relative project-image-container">
          {isExternalImage ? (
            // For external images, use next/image with domains configured
            <div className="relative w-80 h-64 overflow-hidden rounded-lg">
              <Image
                src={image.src}
                alt={name}
                fill
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                className="object-cover transition-opacity duration-[0.7s] delay-[0.3s] group-hover:opacity-0"
                unoptimized={isExternalImage} // Skip optimization for external images
              />
              {demo && (
                <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Link 
                    href={demo} 
                    target="_blank"
                    className="bg-[#16f2b3] text-gray-900 px-4 py-2 rounded-md flex items-center gap-2 hover:bg-white transition-colors duration-300"
                  >
